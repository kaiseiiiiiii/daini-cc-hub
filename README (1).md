...
**Thanks だけは、宛先本人を @メンションします。** 一括通知はスペース全員に同じ1通が
届くだけで、感謝された本人がそれに気づけないためです（スペースへの通知自体は残します。
Thanks は人目に触れること自体が価値なので、本人あてに切り替えると承認の意味が消えます）。
Chat はメールアドレスでメンションできないので、`members.chatUserId` に Chat のユーザーIDを
入れておく必要があります。取り込み手順は [`docs/sync-setup.md`](docs/sync-setup.md) を参照してください。
未設定の人は今までどおり表示名で出るだけで、通知は止まりません。
...
|---|---|---|---|
| `members` | 小文字のメールアドレス | 不可（コンソールのみ。`chatUserId` だけ GAS が更新） | 許可リスト兼メンバーマスタ |
...
| `memberKpi` | `current` | GAS のみ | メンバー別の対応数・手配数・手配率・手配粗利額。**`canSeeTeamKpi` を持つ人しか読めない**（唯一の非公開コレクション） |
